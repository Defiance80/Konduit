...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ route('agency.clients.update', $client) }}" class="space-y-5">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="status" class="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="address" value="{{ old('address', $client->address) }}"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" placeholder="123 Main St, City, State 00000">
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="contact_person" value="{{ old('contact_person', $client->contact_person) }}"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" placeholder="Jane Smith, Marketing Director">
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Contact Email</label>
                    <input type="email" name="contact_person_email" value="{{ old('contact_person_email', $client->contact_person_email) }}"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Contact Phone</label>
                    <input type="text" name="contact_person_phone" value="{{ old('contact_person_phone', $client->contact_person_phone) }}"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 space-y-4">
            ...
            $selected = old('services_interested', $client->services_interested ?? []);
            @endphp
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                ...
                <label class="flex items-center gap-2 cursor-pointer rounded-xl border px-3 py-2.5 text-sm transition-all hover:border-brand-300 hover:bg-brand-50/50 dark:hover:border-brand-700 dark:hover:bg-brand-500/5">
                    <input type="checkbox" name="services_interested[]" value="{{ $val }}" {{ in_array($val, (array)$selected) ? 'checked' : '' }}
