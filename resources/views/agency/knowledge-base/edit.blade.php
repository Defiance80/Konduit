...
<div class="max-w-3xl space-y-6">
    <div class="flex items-center gap-4">
        <a href="{{ route('agency.knowledge-base.show', $article) }}" class="text-gray-400 hover:text-gray-600">
        ...
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Article</h1>
    </div>

    <form action="{{ route('agency.knowledge-base.update', $article) }}" method="POST" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 p-6 space-y-5">
        @csrf @method('PUT')
        <div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $article->category) }}" list="kb-cats"
                    class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                <datalist id="kb-cats">
            ...
            <div class="flex items-end pb-1">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_public" value="1" {{ old('is_public', $article->is_public) ? 'checked' : '' }}
        ...
        <div>
            ...
            <textarea name="content" rows="16" required
                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-mono dark:border-gray-700 dark:bg-gray-800 dark:text-white">{{ old('content', $article->content) }}</textarea>
        </div>
        <div class="flex gap-3 pt-2">
            <a href="{{ route('agency.knowledge-base.show', $article) }}" class="rounded-lg border border-gray-200 px-6 py-2.5 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">Cancel</a>
